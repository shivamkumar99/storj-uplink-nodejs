...
const DOWNLOAD_PATH = process.env.DOWNLOAD_FILE_PATH || path.resolve(__dirname, 'downloaded.txt');
...
async function main() {
  ...
  console.log(`Downloaded ${OBJECT_KEY} to ${DOWNLOAD_PATH} (${result.bytesRead} bytes)`);
  await project.close();
}
