...
/* Build provenance, generated by `make record-uplink-c-version`. Older build
 * trees may not have it, so fall back to "unknown" rather than fail. */
#if defined(__has_include)
#  if __has_include("uplink_c_version.h")
#    include "uplink_c_version.h"
#  endif
#endif
#ifndef UPLINK_C_REF
#  define UPLINK_C_REF "unknown"
#endif
#ifndef UPLINK_C_MODULE_VERSION
#  define UPLINK_C_MODULE_VERSION "unknown"
#endif
#ifndef UPLINK_C_VCS_REVISION
#  define UPLINK_C_VCS_REVISION "unknown"
#endif
#ifndef STORJ_UPLINK_VERSION
#  define STORJ_UPLINK_VERSION "unknown"
#endif
...
static napi_value Init(napi_env env, napi_value exports) {
    ...
    /* Expose which uplink-c / storj.io/uplink this addon was built against */
    {
        const char* names[]  = { "uplinkCRef", "uplinkCVersion", "uplinkCRevision", "storjUplinkVersion" };
        const char* values[] = { UPLINK_C_REF, UPLINK_C_MODULE_VERSION, UPLINK_C_VCS_REVISION, STORJ_UPLINK_VERSION };
        for (size_t i = 0; i < sizeof(names) / sizeof(names[0]); i++) {
            napi_value str;
            if (napi_create_string_utf8(env, values[i], NAPI_AUTO_LENGTH, &str) == napi_ok) {
                napi_set_named_property(env, exports, names[i], str);
            }
        }
    }
