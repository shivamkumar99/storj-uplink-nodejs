...
# Exact uplink-c ref to build: a tag (v1.14.1) or a full commit SHA. Source and
# hybrid builds and the prebuilds workflow all use this value, so a given
# package version always wraps the same libuplink. Bump deliberately.
#   fa48e8c862039e0fd8936d08f623bc898a6a579b = uplink-c main @ 2026-08-24 -> storj.io/uplink v1.14.3
UPLINK_C_VERSION ?= fa48e8c862039e0fd8936d08f623bc898a6a579b

# Generated at build time; compiled into the addon and exposed as
# uplinkCVersion() from TypeScript.
UPLINK_C_VERSION_HEADER := $(INCLUDE_DIR)/uplink_c_version.h
...
install-source: check-go check-git check-compiler check-python $(PLATFORM_DIR) $(INCLUDE_DIR)
	...
	@echo "--------------------------------------------------------"
	@echo "  uplink-c : $(UPLINK_C_SRC_DIR) @ $(UPLINK_C_VERSION)"
	@echo "  Dest     : $(PLATFORM_DIR)"
	@echo ""
	$(Q)echo "[1/3] Fetching storj/uplink-c @ $(UPLINK_C_VERSION) ..."
	$(Q)if [ ! -d "$(UPLINK_C_SRC_DIR)/.git" ]; then \
		git init -q "$(UPLINK_C_SRC_DIR)" && \
		git -C "$(UPLINK_C_SRC_DIR)" remote add origin "$(UPLINK_C_REPO)" ; \
	fi
	$(Q)git -C "$(UPLINK_C_SRC_DIR)" fetch -q --depth 1 origin "$(UPLINK_C_VERSION)"
	$(Q)git -C "$(UPLINK_C_SRC_DIR)" checkout -q --detach FETCH_HEAD
	...
	@$(MAKE) record-uplink-c-version
	@$(MAKE) verify-uplink-c-version
...
# uplink-c provenance
#
# record-uplink-c-version: read the module info Go embedded in libuplink and
# write it as a header that addon.c compiles in (exposed as uplinkCVersion()).
# verify-uplink-c-version: fail the build if the library was not built from
# UPLINK_C_VERSION — catches a stale .uplink-c-build checkout or a wrong pin.
# =============================================================================
.PHONY: record-uplink-c-version verify-uplink-c-version print-uplink-c-version

record-uplink-c-version:
	$(Q)modinfo="$$(go version -m "$(PLATFORM_DIR)/$(LIB_NAME)")" ; \
	mod="$$(printf '%s\n' "$$modinfo" | awk '$$1=="mod" && $$2=="storj.io/uplink-c" {print $$3}')" ; \
	dep="$$(printf '%s\n' "$$modinfo" | awk '$$1=="dep" && $$2=="storj.io/uplink" {print $$3}')" ; \
	rev="$$(printf '%s\n' "$$modinfo" | awk -F= '$$1 ~ /vcs\.revision$$/ {print $$2}')" ; \
	printf '/* Generated by the Makefile (record-uplink-c-version) - do not edit. */\n#ifndef UPLINK_C_VERSION_H\n#define UPLINK_C_VERSION_H\n#define UPLINK_C_REF "%s"\n#define UPLINK_C_MODULE_VERSION "%s"\n#define UPLINK_C_VCS_REVISION "%s"\n#define STORJ_UPLINK_VERSION "%s"\n#endif\n' \
		"$(UPLINK_C_VERSION)" "$${mod:-unknown}" "$${rev:-unknown}" "$${dep:-unknown}" > "$(UPLINK_C_VERSION_HEADER)" ; \
	echo "  Provenance -> $(UPLINK_C_VERSION_HEADER) (uplink-c $${mod:-?} @ $${rev:-?}, storj.io/uplink $${dep:-?})"

verify-uplink-c-version:
	$(Q)modinfo="$$(go version -m "$(PLATFORM_DIR)/$(LIB_NAME)")" ; \
	rev="$$(printf '%s\n' "$$modinfo" | awk -F= '$$1 ~ /vcs\.revision$$/ {print $$2}')" ; \
	mod="$$(printf '%s\n' "$$modinfo" | awk '$$1=="mod" && $$2=="storj.io/uplink-c" {print $$3}')" ; \
	want="$(UPLINK_C_VERSION)" ; \
	case "$$want" in \
		v*) got="$$mod" ;; \
		*)  got="$$rev" ;; \
	esac ; \
	case "$$got" in \
		"$$want"*) echo "  Verified   libuplink was built from uplink-c $$want" ;; \
		*) echo "ERROR: libuplink reports uplink-c '$$mod' @ '$$rev', expected '$$want' (UPLINK_C_VERSION)" ; exit 1 ;; \
	esac

print-uplink-c-version:
	@echo "$(UPLINK_C_VERSION)"
